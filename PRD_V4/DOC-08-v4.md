...
> **前置依赖**: DOC-01 v4(Schema: im_bindings + im_channel_configs + **im_message_dedup**), DOC-07 v4(TaskService + SessionService)
> **Phase**: 2(后端功能模块)
> **Task 数**: 3
> **v4 变更摘要**: 基于 5 轮 review 修订,12 处精确修补(详见文末 §附录 A)。核心修订:**IM Webhook 幂等必做**(im_message_dedup 表防止同一消息因 IM 平台重试被处理多次)、**im_bindings 唯一约束放宽**(支持同 Prism 用户在同一平台绑定多个群聊)、结构化日志 + Prometheus metrics。ADR 编号从 ADR-070 接续至 ADR-073。
> **审计关注点**:
> - **与 Web 端同优先级设计**:IM 消息不是"降级入口",走完全相同的 TaskService.submit() → RunLifecycle → Harness Runtime → Agent 链路
> - **三平台共用消息路由和 session 管理抽象层**:抽象出 `IMAdapter` 接口(接收/发送/绑定),三个平台实现该接口
> - **v4 Webhook 幂等**:IM 平台通常会在超时无响应时重试 webhook(如飞书 3s 超时重试最多 3 次)。若不做幂等,同一条用户消息可能被处理 2-4 次,造成重复 Run。必须基于 `(channel, msg_id)` 做幂等(im_message_dedup 表或 Redis SETNX)
...
- **ADR-070(IM Webhook 幂等必做)**:所有 IM 平台的 receive() 入口在调用 IMGateway.route() 前,必须先查 `im_message_dedup` 表:
  - 命中 → ignore,返回 200 OK(让 IM 平台不再重试)
  - 未命中 → INSERT `(channel, msg_id, received_at, user_id)` 后继续处理
  - 过期记录(> 7 天)定期清理(Cron)

  或使用 Redis 方案:`SETNX im:dedup:{channel}:{msg_id} 1 EX 604800`(7 天),返回 0 表示已存在。两方案在 Phase 1 二选一(推荐 DB,便于审计)。
...
- **v4:所有 IMAdapter.receive() 在调用 IMGateway.route() 前查 im_message_dedup 表,命中 → ignore**
...
│   └── im_dedup.py            # v4 新文件:Webhook 幂等
└── schemas/
    └── im.py                  # IM 相关 Schema
```

### 0. app/services/im_dedup.py(v4 新文件)

```python
"""
IM Webhook 幂等服务(v4 ADR-070)
...
from app.models.im import IMMessageDedup
...
class IMDedupService:
    ...
    def is_duplicate(self, channel: str, msg_id: str, user_id: str | None = None) -> bool:
        """
        返回 True 表示已处理过该消息,调用方应 ignore。
        返回 False 表示首次,已记录 dedup 条目,调用方继续处理。
        """
        try:
            self._db.add(IMMessageDedup(
                channel=channel,
                msg_id=msg_id,
                received_at=datetime.now(timezone.utc),
                user_id=user_id,
            ))
            self._db.flush()  # trigger unique constraint check
            return False
    ...
    def cleanup_expired(self, days: int = 7) -> int:
        """定期清理过期 dedup 条目(Cron 或定时任务调用)"""
        threshold = datetime.now(timezone.utc) - timedelta(days=days)
        deleted = self._db.query(IMMessageDedup)\
            .filter(IMMessageDedup.received_at < threshold).delete()
        self._db.commit()
        return deleted


# 可选 Redis 方案(Phase 1 与 DB 二选一)
class IMDedupRedisService:
    ...
    async def is_duplicate(self, channel: str, msg_id: str) -> bool:
        ...
        # SETNX 返回 1 表示首次设置(未重复);0 表示已存在(重复)
...
class IMGateway:
    ...
    async def _handle_message(self, msg: IMIncomingMessage) -> None:
        """
        统一消息处理。v4:入口先查幂等,命中则 ignore。

        这是所有 IM 消息的唯一入口。
        """
        with self._db_factory() as db:
            # v4 ADR-070:Webhook 幂等
            msg_id = msg.raw.get("message_id") or msg.raw.get("msg_id")
            if msg_id:
                dedup = IMDedupService(db)
                if dedup.is_duplicate(msg.channel, msg_id):
                    logger.info("im.message.duplicate",
                                channel=msg.channel, msg_id=msg_id)
...
- **ADR-071(im_bindings 三元组唯一约束支持多群聊)**:原 v3.1 约束为 `UNIQUE(channel, platform_user_id)`,即一个 IM 用户在一个平台只能绑定一次。但实际场景:同一个 Prism 用户可能在飞书的"工作群"和"家庭群"都要接入(不同 chat_id)。v4 修订为 `UNIQUE(channel, platform_user_id, platform_chat_id)` 三元组。同一 IM 用户的不同 chat_id 对应不同 binding 记录。来源:Batch 3 §B3-IV。

#### 验收标准(v4 扩展)

- Web 端可以生成配对码(6 位随机码,5 分钟过期)
- IM 端发送配对码后完成绑定
- **v4:同一 IM 用户可以在同一平台绑定多个不同的群聊/会话**(三元组唯一而非二元组)
- 已绑定的 IM 用户可以解除绑定
- 配对码过期后不可使用
- 配对码使用后不可重复使用
...
> **文档维护说明(v4)**:本文档的 3 个 Task 完成后,Prism v2 将拥有完整的 IM 接入能力:IMAdapter 抽象接口 + IMGateway 统一路由(与 Web 端共享 TaskService 链路)+ **Webhook 幂等(im_message_dedup 表)** + 飞书/企业微信/Telegram 三平台适配器 + 配对码用户绑定(**支持多群聊三元组唯一**)+ 结构化日志 + Prometheus metrics。
> **最后更新**: 2026-04-18 (v4 review 修订版) | **下一步**: DOC-09 v4 Backend MCP-Provider-Admin

---

## 附录 A: v4 修订清单

本次修订共 12 处精确修补,对应 Batch 1-5 review + PDF 补丁:

| # | 位置 | 修订内容 | 来源 |
|---|---|---|---|
| 1 | Header | 版本 3.1 → 4.0,日期 2026-04-18,前置依赖 v4 + im_message_dedup,v4 摘要段 | 全局 |
| 2 | Task 8.1 Part A | 新增 ADR-070(IM Webhook 幂等必做) | Batch 3 B3-5 |
| 3 | Task 8.1 im_dedup.py(新文件) | `IMDedupService.is_duplicate(channel, msg_id)` + 批量清理过期记录;Redis 方案 `IMDedupRedisService` | Batch 3 B3-5 |
| 4 | Task 8.1 im_gateway.py `_handle_message` | 入口先查幂等,命中 → ignore + 记 Prometheus counter | 同上 |
...
| 7 | Task 8.3 Part A 修订 | ADR-071:im_bindings 唯一约束三元组 → 支持多群聊绑定 | Batch 3 §B3-IV |
| 8 | Task 8.3 pairing 逻辑 | platform_chat_id 不同 → 不同 binding 记录 | 同上 |
| 9 | 结构化日志 | im.message.received / im.message.duplicate / im.webhook.failed / im.binding.paired / im.binding.unpaired | 全局 §3.4 |
| 10 | Prometheus metrics | `prism_im_messages_total{channel}` / `prism_im_webhook_duplicates_total{channel}` / `prism_im_bindings_active` | Batch 5 §B5-I |
| 11 | ADR 编号 ADR-070~073 + 交叉引用 v3 → v4 | 全局 | 全局 |
| 12 | 附录 A + 文末 | 修订清单 + 下一步 DOC-09 v4 | SOP |  
