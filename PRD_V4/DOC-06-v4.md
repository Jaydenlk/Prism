...
> **前置依赖**: DOC-01 v4(Schema: users + invite_codes), DOC-02 v4 Task 2.1(项目骨架 + ORM 模型已就绪)
> **Phase**: 2(后端功能模块)
> **Task 数**: 2
> **v4 变更摘要**: 基于 5 轮 review 修订,10 处精确修补(详见文末 §附录 A)。核心修订:**三密钥独立**(JWT_SECRET / ENCRYPTION_KEY / CALLBACK_SECRET,启动校验互不相等)、**SSE ticket 一次性认证**(替代 URL query JWT 泄露风险)、审计日志结构化。ADR 编号从 ADR-050 接续至 ADR-055。
> **审计关注点**:
> - **v4 三密钥独立**:`JWT_SECRET`(JWT HMAC 签名)/ `ENCRYPTION_KEY`(Provider API Key AES-256-GCM 加密)/ `CALLBACK_SECRET`(子进程回调 HMAC)。三者必须不同,main.py lifespan 启动时校验。共用会导致:轮换 JWT 时 Provider API Key 全部失效 / 或不敢轮换 JWT
> - **v4 SSE ticket**:SSE 连接从 URL query JWT 改为一次性 ticket(60s TTL,Redis 存储,GETDEL 原子消费),避免 token 泄露到 browser history/referer/server log
...
- **ADR-050(三密钥独立 + 启动校验)**:`JWT_SECRET` / `ENCRYPTION_KEY` / `CALLBACK_SECRET` 三者必须独立:
  - `JWT_SECRET`(HMAC-SHA256):JWT 签名。轮换时只影响已签发的 token(用户重新登录即可)
  - `ENCRYPTION_KEY`(AES-256-GCM):Provider API Key 加密。轮换时需要重新加密所有已存储的 key(高成本)
  - `CALLBACK_SECRET`(HMAC-SHA256):子进程 → Backend 回调的 `X-Callback-Secret` 头。轮换时需要重启所有子进程
  - main.py lifespan 启动时:若三者任意两个相等,fast fail raise RuntimeError
  - `.env` 中三者各自配置(64 hex chars 随机)

  来源:Batch 1 v2 §3.6, Batch 3 §A6-1。

- **ADR-051(SSE ticket 替代 URL query JWT)**:SSE 连接原设计 `?token=<JWT>` 查询参数,存在以下风险:
  - Token 泄露到 browser history
  - Token 泄露到 Referer header 发送给第三方资源
  - Token 泄露到 Nginx access_log

  v4 修订:**一次性 ticket**。流程:
  1. 前端 `POST /auth/sse-ticket {session_id}` → Backend 生成 uuid7 ticket,`SETEX sse_ticket:{ticket} 60 {user_id,session_id}`
  2. 前端 `new EventSource("/sessions/{id}/stream?ticket=X&last_event_id=Y")`
  3. Backend SSE handler 收到 → `GETDEL sse_ticket:{ticket}` 原子消费(防重放)
  4. 校验 session_id 匹配 → 建立 SSE 连接
  5. Ticket 60s 内未使用即过期

  来源:Batch 1 v2 §3.6, Batch 3 §A6-1/A6-2。

- **ADR-052(Refresh token HttpOnly cookie)**:Refresh token 存储在 HttpOnly cookie 中,不存储在 DB。本身是 JWT(含 user_id + exp),签名验证即可确认有效性。登出通过前端删除 cookie + 可选的 token blacklist(Phase 1 不实现 blacklist)。

#### Harness 层交互

认证模块不直接与 Harness Runtime 交互(Harness 运行在 CLI 子进程中,认证运行在 Backend 进程中)。但认证产生的 `user_id` 会传递到 Run 配置中,Harness 的数据隔离护栏(铁律 4)依赖 `user_id` 做 WHERE 过滤。SSE ticket 机制由 Backend 的 sse_manager 消费(DOC-07 v4 Task 7.3)。
...
- **v4:main.py lifespan 调用 `validate_secrets()` 校验三密钥独立;任意两个相等则 fast fail**
- **v4:`POST /auth/sse-ticket` 端点接受 `{session_id}`,返回 `{ticket, expires_at}`(60s TTL)**
- **v4:ticket 只能消费一次**(GETDEL 原子操作)
- **v4:ticket 的 session_id 必须匹配 SSE 连接 URL 的 session_id**,不匹配 403
...
### 1. app/core/config.py 修改(v4:三密钥)

新增配置项:
```python
ENCRYPTION_KEY: str             # AES-256-GCM 加密 Provider API Key(64 hex chars = 32 bytes)
CALLBACK_SECRET: str            # 子进程回调 HMAC(64 hex chars)
SSE_TICKET_TTL_SECONDS: int = 60  # SSE 一次性 ticket TTL
```

在 `.env.example` 中同步新增:
```bash
JWT_SECRET=<random-64-hex-chars-A>
ENCRYPTION_KEY=<random-64-hex-chars-B>      # 必须与 JWT_SECRET 不同
CALLBACK_SECRET=<random-64-hex-chars-C>     # 必须与 JWT_SECRET / ENCRYPTION_KEY 不同
```

### 1.1 app/main.py lifespan 加入 validate_secrets(v4 新增)
...
def validate_secrets(settings) -> None:
    ...
    for name, val in secrets.items():
        if not val or len(val) < 32:
            raise RuntimeError(f"{name} missing or too short(需 ≥ 32 字符)")
    if len(set(secrets.values())) != 3:
        dup = [n for n in secrets if list(secrets.values()).count(secrets[n]) > 1]
        raise RuntimeError(f"三密钥必须互不相等,发现重复: {dup}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    validate_secrets(settings)  # 启动时 fast fail
    # ... 其他启动初始化(Redis 连接 / HeartbeatMonitor / Prometheus registry)
    yield
    # ... 清理
```
...
### 8. app/main.py 修改(v4:三密钥校验 + SSE ticket 服务)

```python
@asynccontextmanager
async def lifespan(app: FastAPI):
    # v4:启动校验三密钥独立(fast fail)
    ...
    # v4:启动 HeartbeatMonitor(DOC-07 v4 Task 7.3)
...
### 9. app/services/sse_ticket_service.py(v4 新文件)

```python
"""
SSE Ticket Service - 一次性 SSE 认证 ticket(v4 ADR-051)
"""
...
class SSETicketService:
    ...
    async def verify_and_consume(self, ticket: str, session_id: str) -> str:
        """验证并消费 ticket(原子 GETDEL),返回 user_id 或抛 401/403"""
        key = f"sse_ticket:{ticket}"
...
### 10. app/api/v1/auth.py 新增 `/sse-ticket` 端点(v4)

```python
@router.post("/sse-ticket", response_model=dict)
async def create_sse_ticket(
    body: SSETicketRequest,                            # {session_id: str}
    user: User = Depends(get_current_user),
    redis_client = Depends(get_redis),
    settings = Depends(get_settings),
):
    """v4:生成一次性 SSE ticket(60s 有效)"""
    # 校验 session_id 属于当前用户
    session = db.query(Session).filter_by(id=body.session_id, user_id=user.id).first()
    if not session:
        raise HTTPException(404, "Session not found")

    service = SSETicketService(redis_client, settings)
    result = await service.generate_ticket(str(user.id), body.session_id)

    # 审计日志(v4 结构化)
    logger.info("auth.sse_ticket.issued",
                user_id=str(user.id),
                session_id=body.session_id,
                ticket_prefix=result["ticket"][:8])
    prism_sse_tickets_issued_total.inc()
...
> **文档维护说明(v4)**:本文档的 2 个 Task 完成后,Prism v2 将拥有完整的认证和用户管理能力:JWT 双 token 认证 + bcrypt 密码存储 + AES-256-GCM 数据加密(**三密钥独立:JWT_SECRET / ENCRYPTION_KEY / CALLBACK_SECRET**)+ **SSE 一次性 ticket 认证** + 邀请码注册控制 + Admin 用户/邀请码/用量/审计管理(结构化日志)。这是 DOC-07 v4(Session/Run/Task)和 DOC-09 v4(MCP/Provider/Admin)的前置依赖。
> **最后更新**: 2026-04-18 (v4 review 修订版) | **下一步**: DOC-07 v4 Backend Session-Run-Task

---

## 附录 A: v4 修订清单

本次修订共 10 处精确修补,对应 Batch 1-5 review + PDF 补丁:

| # | 位置 | 修订内容 | 来源 |
|---|---|---|---|
| 1 | Header | 版本 3.1 → 4.0,日期 2026-04-18,v4 摘要段,审计关注点升级为三密钥 + SSE ticket | 全局 |
| 2 | Task 6.1 Part A | 新增 ADR-050(三密钥独立 + 启动校验)/ADR-051(SSE ticket 替代 URL query JWT)/ADR-052(Refresh token HttpOnly cookie) | Batch 1 v2 §3.6, Batch 3 §A6-1 |
| 3 | Task 6.1 验收标准 | 加 main.py lifespan `validate_secrets()`;`POST /auth/sse-ticket` 端点;ticket 只能消费一次(GETDEL);session_id 匹配校验 | 同上 |
| 4 | Task 6.1 config.py | 新增 CALLBACK_SECRET + SSE_TICKET_TTL_SECONDS 配置;.env.example 三密钥独立注释 | 同上 |
| 5 | Task 6.1 main.py lifespan | 加入 `validate_secrets()` fast fail(Start section 1.1) | Batch 3 §A6-1 |
| 6 | Task 6.1 sse_ticket_service.py(新文件) | `generate_ticket(user_id, session_id) -> {ticket, expires_at}` + `verify_and_consume(ticket, session_id) -> user_id`(GETDEL 原子) | Batch 3 §A6-2 |
| 7 | Task 6.1 api/v1/auth.py 新端点 | `POST /auth/sse-ticket` — Body `{session_id}`, Return `{ticket, expires_at}` + 审计日志 + Prometheus metric | 同上 |
| 8 | Task 6.1 security.py encrypt/decrypt | 完整 AES-256-GCM 实现(用 `ENCRYPTION_KEY`,不是 `JWT_SECRET`),引用 DOC-02 v4 Task 2.3 | DOC-02 Task 2.3 引用 |
| 9 | Task 6.2 Part A | 审计日志结构化(structlog + event_type + severity),邀请码逻辑保留;Prometheus:`prism_auth_invite_code_used_total` / `prism_auth_login_total` / `prism_sse_tickets_issued_total` | Batch 3 §A6-3, Batch 5 §B5-I |
| 10 | ADR 编号 ADR-050~055 + 附录 A + 文末 + 交叉引用 v3 → v4 | 全局 | SOP |  
