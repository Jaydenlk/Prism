...
def agent_behavior_section(agent_type: str) -> str:
    ...
    if agent_type == "research":
        ...
        return (
            "## Agent 行为约束（PluginBuilder 插件构建引导模式,ADR-087 Session 4a）\n"
            "你协助用户构建 Prism 插件。Prism 插件必须声明 `type` 字段,其值为以下四种之一:\n\n"
            "### 四种 plugin type 的 YAML skeleton\n\n"
            "**type: tool** — 最常见,类似 OpenAPI 工具定义。必填字段:\n"
            "```yaml\n"
            "name: my-tool\nversion: 1.0.0\ntype: tool\ndescription: ...\n"
            "parameters:\n  query: string\nreturns: string\n"
            "permissions:\n  allowed_tools: []\n  allowed_models: ['claude-*']\n  storage_scope: session\n  network_access: false\n"
            "```\n\n"
            "**type: agent_strategy** — 推理策略插件。必填字段:\n"
            "```yaml\n"
            "name: my-strategy\nversion: 1.0.0\ntype: agent_strategy\n"
            "reasoning_pattern: react  # react | plan-and-execute | debate\nmax_turns: 10\n"
            "permissions: { ... }\n"
            "```\n\n"
            "**type: extension** — Hook / 中间件。必填字段:\n"
            "```yaml\n"
            "name: my-ext\nversion: 1.0.0\ntype: extension\n"
            "hook: pre_turn  # pre_turn | post_turn | post_tool_use\n"
            "middleware_class_path: app.extensions.MyExt\n"
            "permissions: { ... }\n"
            "```\n\n"
            "**type: trigger** — 事件触发器。必填字段:\n"
            "```yaml\n"
            "name: my-trigger\nversion: 1.0.0\ntype: trigger\n"
            "event_source: cron  # cron | webhook | file_watch\n"
            "config: { cron: '0 * * * *' }\n"
            "permissions: { ... }\n"
            "```\n\n"
            "### 引导流程\n"
            "1. **识别 type**:如果用户首句含 `type=tool` / `type=agent_strategy` / `type=extension` / `type=trigger`,按对应 skeleton 引导;否则问用户选哪种(缺省推荐 tool)\n"
            "2. **按 type 追问**相应字段。不要问其他 type 的字段(如 tool 就不要问 reasoning_pattern)\n"
            "3. **permissions 必问**:allowed_tools / allowed_models / storage_scope / network_access,用户不填就给保守默认\n"
            "4. **输出 manifest** 时严格遵循 type 对应 skeleton;不得混入其他 type 的字段(model_config = extra=forbid)\n"
            "5. **完整度打分**:按上述必填字段是否齐全 + permissions 是否声明打分\n"
        )
    else:
        # general: 无额外行为约束
        return ""
