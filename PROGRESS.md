...
| DOC-06 Task 6.2: 用户管理 + 邀请码 | completed | 2026-04-19 | 2026-04-19 | e47c31d | 4新文件+1修改: schemas/invite.py(CreateInviteCodeRequest+InviteCodeResponse is_valid); schemas/user.py(UserListResponse+UpdateUserRoleRequest Literal); services/invite_service.py(PRISM-格式 generate_code/create/validate/consume/revoke); api/v1/admin.py(7端点 router-level require_admin); api/v1/__init__注册admin_router; ADR-059落地; 全部验证PASS; DOC-06完整收官 |
...
| DOC-08 Task 8.3: 用户绑定(三元组) | completed | 2026-04-19 | 2026-04-19 | 177ee65 | 1新文件+2修改: im_binding_service.py(IMBindingService 4方法+expires_at+碰撞重试+TTL校验+ADR-071三元组IntegrityError捕获); im.py(3端点委托service，移除内联逻辑); im_gateway.py(_handle_pairing重构调用service.pair()+platform_chat_id); 12项验证全PASS; DOC-08完整收官3/3 |
...
> **最后更新**: 2026-04-19（DOC-12 Task 12.4 完成: Prometheus 68 metrics + 4 Grafana dashboards ADR-116; 38/51 Task 完成）
> **下一个动作**: DOC-12 Task 12.5 — OTel Tracing(跨进程 W3C)
